...
void alarm_handler(int sig) {
    // if timeout, print an error message
    fprintf(stdout, "request timeout; terminating...\n");
    fflush(stdout);
    exit(0);
}
...
int main(int argc, char *argv[]) {
    ...
    if (recv_len == OVERLAYACKSIZE && recv_buf[0] == OVERLAYACKANS) {
        ...
        fprintf(stdout, "the request has been rejected; terminating...\n");
        fflush(stdout);
    }
